...
http_check() {
    local target=$1
    echo "===== 正在检测HTTP服务：$target ====="
    echo "检测时间：$(date)"

    # 获取响应状态
    local response=$(curl -I -s --connect-timeout $TIMEOUT "$target" | head -n1)
    ...
    # 检测响应时间
    local time_cost=$(curl -o /dev/null -s --connect-timeout $TIMEOUT -w "%{time_total}\n" "$target")
    echo "响应时间：${time_cost} 秒"
    echo "===== 检测完成 ====="
}
