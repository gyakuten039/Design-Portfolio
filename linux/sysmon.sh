...
# 安全日志报表：日志统计 + TopN IP + 终端输出 (去绝对路径版)

SCRIPT_NAME="$(basename "$0")"                                  # 脚本名称
DEFAULT_CONFIG="./logreport.conf"                               # 默认配置文件（当前目录）
...
pick_logfile_whiptail() {
  local sample="./sample_auth.log"  # 兜底样例（当前目录）
  local default_path="/var/log/auth.log"
...
main() {
  ...
  [[ -f "$logfile" ]] || logfile="./sample_auth.log"
  local top_n=""
  ...
  local cmd
  if (( use_menu == 1 )); then
    cmd="./$SCRIPT_NAME -w -c $cfg -n $top_n"
  else
    cmd="./$SCRIPT_NAME -c $cfg -l $logfile -n $top_n"
  fi
