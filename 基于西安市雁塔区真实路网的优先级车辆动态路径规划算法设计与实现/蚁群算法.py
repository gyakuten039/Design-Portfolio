...
class Vehicle:
    ...
    def update_position(self, current_time, network):
        ...
        # 获取边数据
        edge_data = network.edges.get((u, v)) or network.edges.get((v, u))
        
        if not edge_data:
            # 如果找不到边数据，直接跳到下一个节点
            print(f"  {self.vehicle_id}: 跳过无法找到的边 {u}->{v}")
...
class NetworkWrapper:
    ...
    def _extract_data(self):
        ...
        for u, v, data in self._nx_graph.edges(data=True):
            ...
            print(f"路段 {u}-{v}: 长度={length}m, 限速={speed_limit}km/h")
...
class PathSearchAgent:
    ...
    def move_to(self, next_node, cost):
        """移动到下一个节点"""
...
class IntelligentPathPlanner:
    ...
    def __init__(self, network, learning_rate=0.1, exploration_rate=0.3, convergence_threshold=0.01, 
                 max_iterations=100, search_agents=20):
        self.network = network
        self.learning_rate = learning_rate  # 学习率（信息素更新强度）
        self.exploration_rate = exploration_rate  # 探索率（随机选择倾向）
        self.convergence_threshold = convergence_threshold  # 收敛阈值
        self.max_iterations = max_iterations  # 最大迭代次数
        self.search_agents = search_agents  # 搜索代理数量
        ...
        for i, u in enumerate(nodes):
            for j, v in enumerate(nodes):
                if i != j:
                    self.path_memory[(u, v)] = 1.0  # 初始路径记忆强度
                    self.distance_heuristic[(u, v)] = self._calculate_distance_heuristic(u, v)
    
    def _calculate_distance_heuristic(self, u, v):
        """计算距离启发式信息（距离的倒数）"""
        edge_data = self.network.edges.get((u, v)) or self.network.edges.get((v, u))
        if edge_data:
            length = edge_data.get('length', 300)
            return 1.0 / length  # 距离越短，启发式信息越大
        return 0.01  # 默认值
    
    def _get_available_routes(self, node, visited):
        """获取可用的路线（邻居节点）"""
        routes = []
        for (u, v), edge_data in self.network.edges.items():
            if u == node and v not in visited:
                routes.append((v, edge_data))
            elif v == node and u not in visited:
                routes.append((u, edge_data))
        return routes
    
    def _calculate_route_preference(self, current_node, next_node, visited, vehicle=None):
        """计算路线选择偏好度"""
        # 获取路径记忆强度
        memory_strength = self.path_memory.get((current_node, next_node), 0.1)
        
        # 获取距离启发式信息
        distance_heuristic = self.distance_heuristic.get((current_node, next_node), 0.01)
        
        # 获取路况信息
        edge_data = self.network.edges.get((current_node, next_node)) or \
                   self.network.edges.get((next_node, current_node))
        
        if edge_data:
            # 考虑拥堵的启发式信息
            load = edge_data.get('load', 0)
            congestion_factor = 1 + 0.3 * (load / 5)  # 拥堵因子
            
            # 基础启发式信息
            base_heuristic = distance_heuristic
            
            # 调整启发式信息（拥堵越大，吸引力越小）
            adjusted_heuristic = base_heuristic / congestion_factor
            
            # 根据车辆优先级调整
            if vehicle:
                weight = vehicle.get_priority_weight()
                # 高优先级车辆更关注时间，低优先级更关注距离
                if vehicle.priority == 1:  # 高优先级
                    time_factor = 1.0 / (1 + congestion_factor * 2)
                    preference = (memory_strength ** 0.7) * (adjusted_heuristic ** 0.3) * time_factor
                else:  # 中低优先级
                    preference = (memory_strength ** 0.5) * (adjusted_heuristic ** 0.5)
            else:
                preference = (memory_strength ** 0.5) * (adjusted_heuristic ** 0.5)
        else:
            preference = 0.01
    ...
    def _select_next_route(self, agent, vehicle=None):
        """选择下一条路线"""
        available_routes = self._get_available_routes(agent.current_node, agent.visited)
        ...
        if vehicle:
            filtered_routes = []
            for next_node, edge_data in available_routes:
                edge = (agent.current_node, next_node)
                if edge not in vehicle.preference_edges:
                    filtered_routes.append((next_node, edge_data))
            available_routes = filtered_routes
        ...
        for next_node, edge_data in available_routes:
            preference = self._calculate_route_preference(agent.current_node, next_node, agent.visited, vehicle)
            route_preferences.append((next_node, preference))
            total_preference += preference
        
        # 归一化偏好度
        if total_preference > 0:
            for i in range(len(route_preferences)):
                route_preferences[i] = (route_preferences[i][0], route_preferences[i][1] / total_preference)
        else:
            # 如果所有偏好度都是0，均匀选择
            uniform_preference = 1.0 / len(route_preferences)
            route_preferences = [(next_node, uniform_preference) for next_node, _ in route_preferences]
        
        # 探索-利用平衡：有一定概率随机选择
        if random.random() < self.exploration_rate:
            # 随机探索
            return random.choice(available_routes)[0]
        else:
            # 贪心选择
            return max(route_preferences, key=lambda x: x[1])[0]
    
    def _update_path_memory(self, agents, best_agent):
        """更新路径记忆（替代信息素更新）"""
        # 路径记忆衰减（模拟遗忘）
        for key in self.path_memory:
            self.path_memory[key] *= (1 - self.learning_rate)
        ...
        for agent in agents:
            if agent.is_complete():
                ...
                for i in range(len(path) - 1):
                    u, v = path[i], path[i+1]
                    # 路径质量越好，记忆增强越多
                    memory_boost = 1.0 / (1 + agent.total_cost / 1000)
                    self.path_memory[(u, v)] += memory_boost
                    self.path_memory[(v, u)] += memory_boost  # 无向图
        
        # 最优路径额外强化
        if best_agent and best_agent.is_complete():
            path = best_agent.get_path()
            for i in range(len(path) - 1):
                u, v = path[i], path[i+1]
                # 双倍记忆强化
                memory_boost = 2.0 / (1 + best_agent.total_cost / 1000)
                self.path_memory[(u, v)] += memory_boost
                self.path_memory[(v, u)] += memory_boost
    
    def find_optimal_path(self, start_node, end_node, vehicle=None):
        """寻找最优路径"""
        if start_node == end_node:
            return {'path': [start_node], 'speeds': [40]}, 0
        ...
        for iteration in range(self.max_iterations):
            ...
            # 创建搜索代理群
            for _ in range(self.search_agents):
                agent = PathSearchAgent(start_node, end_node)
                
                # 搜索路径
                while not agent.is_complete():
                    next_node = self._select_next_route(agent, vehicle)
                    if next_node is None:
                        break
                    
                    # 计算移动代价
                    edge_data = self.network.edges.get((agent.current_node, next_node)) or \
                               self.network.edges.get((next_node, agent.current_node))
                    
                    if edge_data:
                        length = edge_data.get('length', 300)
                        load = edge_data.get('load', 0)
                        congestion_factor = 1 + 0.3 * (load / 5)
                        
                        # 根据车辆优先级调整代价
                        if vehicle:
                            weight = vehicle.get_priority_weight()
                            base_time = length / (edge_data.get('speed_limit', 40) * 1000 / 3600)
                            cost = length * weight + base_time * congestion_factor * (1 - weight)
                        else:
                            cost = length * congestion_factor
                        
                        agent.move_to(next_node, cost)
                    else:
                        break
            ...
            for agent in agents:
                if agent.is_complete() and agent.total_cost < iteration_best_cost:
                    iteration_best_cost = agent.total_cost
            
            # 检查收敛
            if abs(iteration_best_cost - best_cost) < self.convergence_threshold:
                convergence_count += 1
            else:
                convergence_count = 0
            ...
            # 更新全局最优解
            for agent in agents:
                if agent.is_complete() and agent.total_cost < best_cost:
                    best_cost = agent.total_cost
                    best_path = agent.get_path()
                    best_agent = agent
            
            # 更新路径记忆
            self._update_path_memory(agents, best_agent)
            
            # 输出迭代信息
            if iteration % 10 == 0:
                print(f"    迭代 {iteration}: 当前最优代价 = {best_cost:.2f}")
            
            # 检查是否收敛
            if convergence_count >= 10:  # 连续10次迭代没有显著改善
                print(f"    算法收敛于迭代 {iteration}")
                break
        ...
        if best_path:
            ...
            print(f"    智能搜索未能找到从 {start_node} 到 {end_node} 的路径")
            return {'path': [start_node, end_node], 'speeds': [40, 40]}, float('inf')
...
class DynamicTrafficSystem:
    ...
    def __init__(self, network):
        ...
        # 智能路径规划器
        self.path_planner = IntelligentPathPlanner(network)
    
    def intelligent_path_planning(self, start, end, vehicle=None):
        """使用智能路径规划系统"""
        if start == end:
            return {'path': [start], 'speeds': [40]}, 0
        
        print(f"  启动智能路径规划系统...")
        path, cost = self.path_planner.find_optimal_path(start, end, vehicle)
        
        if path:
            return path, cost
        else:
            print(f"  路径规划系统未能找到从 {start} 到 {end} 的路径")
    ...
    def add_manual_vehicle(self, vehicle_id):
        ...
        print(f"\n为车辆 {vehicle_id} 输入信息:")
        print("可用的路口:", ", ".join(all_nodes[:10]))
        if len(all_nodes) > 10:
            print(f"  ... 还有 {len(all_nodes)-10} 个路口")
        ...
        while True:
            ...
            if start_node in all_nodes:
                break
            print(f"  错误: 路口 '{start_node}' 不存在，请重新输入")
        ...
        while True:
            ...
            if end_node in all_nodes:
                ...
                print(f"  错误: 路口 '{end_node}' 不存在，请重新输入")
        ...
        if pref_input == 'y':
            print("  可用的路段:")
            road_list = list(self.network.edges.keys())
            for i, (u, v) in enumerate(road_list[:10]):  # 只显示前10条路段
                print(f"    {i+1}. {u}-{v}")
            if len(road_list) > 10:
                print(f"    ... 还有 {len(road_list)-10} 条路段")
            
            while True:
                road_input = input("  输入要规避的路段编号（用逗号分隔，0结束）: ").strip()
                if road_input == '0':
                    break
                try:
                    indices = [int(x.strip()) for x in road_input.split(',')]
                    for idx in indices:
                        if 1 <= idx <= len(road_list):
                            u, v = road_list[idx-1]
    ...
    def plan_route(self, vehicle_id):
        ...
        # 使用智能路径规划系统
        path_info, cost = self.intelligent_path_planning(vehicle.start_node, vehicle.end_node, vehicle)
        ...
        if path_info and 'path' in path_info:
            ...
            print(f"  路径长度: {len(vehicle.path)}个路口")
            print(f"  总代价: {cost:.2f}")
    ...
    def run_time_step_simulation(self):
        ...
        if builder:
            ...
            for vehicle_id, vehicle in self.vehicles.items():
                vehicles_data.append({
                    ...
                    'path': vehicle.path,
                    'color': 'purple'  # 蚁群算法使用紫色
                })
            
            print("\n显示初始车辆位置...")
            builder.visualize_vehicles_realtime(vehicles_data, "蚁群算法 - 车辆实时位置", simulation_time, blocking=True)
        ...
        while not all_arrived and (simulation_time - self.start_time) < self.max_simulation_time:
            ...
            # 输出当前状态
            print(f"车辆状态: 已到达={arrived_count}, 行驶中={traveling_count}, 等待中={waiting_count}")
        ...
        if builder:
            ...
            print("\n显示最终车辆位置...")
            builder.visualize_vehicles_realtime(vehicles_data, "蚁群算法 - 最终车辆位置", simulation_time, blocking=True)
    ...
    def print_final_summary(self):
        ...
        if not_arrived:
            ...
            for vehicle in not_arrived:
                ...
                if vehicle.status == "traveling":
                    print(f"    当前位置: {vehicle.current_node}, 完成路段: {vehicle.current_edge_index}/{len(vehicle.path)-1}")

# 主函数
def main():
    print("="*60)
    print("智能车辆路径规划系统")
    print("西安市雁塔区交通网络 - 优先级车辆动态调度")
    print("="*60)
    ...
    try:
        ...
        try:
            ...
            print(f"✓ 交通网络构建成功")
            print(f"  路口数: {len(nx_graph.nodes)}")
            print(f"  路段数: {len(nx_graph.edges)}")
            
            # 显示可用路口
            nodes = list(nx_graph.nodes())
            print(f"\n可用路口 (前10个): {', '.join(nodes[:10])}")
            if len(nodes) > 10:
                print(f"  共 {len(nodes)} 个路口")
        ...
        while True:
            try:
                agents_input = input("\n输入搜索代理数量 (10-50, 默认20): ").strip()
                if not agents_input:
                    system.path_planner.search_agents = 20
                    break
                num_agents = int(agents_input)
                if 10 <= num_agents <= 50:
                    system.path_planner.search_agents = num_agents
                    break
                print("错误: 请输入10-50之间的数字")
            except ValueError:
                print("错误: 请输入数字")
        ...
        while True:
            try:
                iter_input = input("输入最大搜索迭代次数 (50-200, 默认100): ").strip()
                if not iter_input:
                    system.path_planner.max_iterations = 100
                    break
                max_iter = int(iter_input)
                if 50 <= max_iter <= 200:
                    system.path_planner.max_iterations = max_iter
                    break
                print("错误: 请输入50-200之间的数字")
            except ValueError:
                print("错误: 请输入数字")
        
        print(f"\n智能路径规划系统参数:")
        print(f"  搜索代理数量: {system.path_planner.search_agents}")
        print(f"  最大迭代次数: {system.path_planner.max_iterations}")
        print(f"  学习率: {system.path_planner.learning_rate}")
        print(f"  探索率: {system.path_planner.exploration_rate}")
        print(f"  收敛阈值: {system.path_planner.convergence_threshold}")
        ...
        # 开始时间步进模拟
        print(f"\n开始交通流模拟，每 {step_seconds} 秒输出一次状态...")
        input("按Enter键开始模拟...")
