...
class DynamicTrafficSystem:
    ...
    def bellman_ford(self, start, end, vehicle=None):
        """Bellman-Ford算法实现，支持优先级和用户偏好"""
        if start == end:
        ...
        for (u, v), edge_data in self.network.edges.items():
            all_edges.append((u, v, edge_data))
            all_edges.append((v, u, edge_data))  # 添加反向边
        ...
        for u, v, edge_data in all_edges:
            if dist[u] != float('inf'):
                length = edge_data.get('length', 100)
                load = edge_data.get('load', 0)
                congestion_factor = 1 + self.ALPHA * (load / 5)
    ...
    def plan_route(self, vehicle_id):
        ...
        # 使用Bellman-Ford算法
        path_info, cost = self.bellman_ford(vehicle.start_node, vehicle.end_node, vehicle)
    ...
    def run_time_step_simulation(self):
        ...
        if builder:
            ...
            for vehicle_id, vehicle in self.vehicles.items():
                vehicles_data.append({
                    ...
                    'path': vehicle.path,
                    'color': 'blue'  # Bellman-Ford使用蓝色
                })
            
            print("\n显示初始车辆位置...")
            builder.visualize_vehicles_realtime(vehicles_data, "Bellman-Ford算法 - 车辆实时位置", simulation_time, blocking=True)
        ...
        if builder:
            ...
            print("\n显示最终车辆位置...")
            builder.visualize_vehicles_realtime(vehicles_data, "Bellman-Ford算法 - 最终车辆位置", simulation_time, blocking=True)
...
def main():
    print("="*60)
    print("车辆路径规划系统 - Bellman-Ford算法实现")
